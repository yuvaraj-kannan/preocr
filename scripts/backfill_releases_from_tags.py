...
def get_commits_since_previous_tag(tag: str) -> list:
    ...
    try:
        ...
        if tag_index == 0:
            ...
            result = subprocess.run(
                ["git", "log", "--format=%s", tag], capture_output=True, text=True, check=True
            )
        else:
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Backfill release files from git tags")
    parser.add_argument(
        "--version",
        help="Specific version to backfill (e.g., v0.5.0). If not provided, backfills all missing versions.",
    )
    parser.add_argument(
        "--repo-url", help="GitHub repository URL (default: auto-detect from git config)"
    )
