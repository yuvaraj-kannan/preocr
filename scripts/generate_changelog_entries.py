...
def get_tag_date(tag: str) -> str:
    ...
    try:
        result = subprocess.run(
            ["git", "log", "-1", "--format=%ai", tag], capture_output=True, text=True, check=True
        )
...
def generate_changelog_entry(version: str, start_tag: str, end_tag: str) -> str:
    ...
    # Categorize commits
    categorized = {"Added": [], "Changed": [], "Fixed": [], "Removed": [], "Security": []}
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Generate CHANGELOG.md entries from git commits")
    parser.add_argument("--version", help="Specific version to generate entry for (e.g., v0.5.0)")
    parser.add_argument(
        "--all", action="store_true", help="Generate entries for all missing versions"
    )
    ...
    if args.version:
        ...
        if version_index == 0:
            print(
                f"Error: {args.version} is the first tag, cannot determine previous tag",
                file=sys.stderr,
            )
            sys.exit(1)
