...
def _extract_page_elements(
    ...
    for idx, block_data in enumerate(merged_blocks):
        ...
        is_bold = (
            block_chars[0].get("fontname", "").endswith("Bold")
            if block_chars and block_chars[0].get("fontname")
            else False
        )
...
def _group_by_lines(items: List[Dict[str, Any]], y_threshold: float = 2.5) -> List[Dict[str, Any]]:
    """
    ...
    if is_rotated:
        ...
        for item in sorted_items:
            ...
            if not placed:
                lines.append(
                    {"x0": item_x, "top": item.get("top", item.get("y0", 0)), "items": [item]}
                )
    else:
        ...
        for item in sorted_items:
            ...
            if not placed:
                lines.append({"top": item_top, "x0": item.get("x0", 0), "items": [item]})
...
def _merge_line_items(
    ...
    for item in items:
        ...
        # - rotation equal
        same_typography = (
            item.get("fontname", "") == current.get("fontname", "")
            and abs(item.get("size", 0) - current.get("size", 0)) < font_size_epsilon
            and item.get("rotation", 0) == current.get("rotation", 0)
        )
        ...
        if is_rotated:
            ...
            current_height = (
                ...
                else current.get("size", 12)
            )
            current_text_len = max(len(current.get("text", "")), 1)
            avg_char_height = (
                current_height / current_text_len if current_height > 0 else item.get("size", 12)
            )
            ...
            current_text_len = max(len(current.get("text", "")), 1)
            avg_char_width = (
                current_width / current_text_len if current_width > 0 else item.get("size", 12)
            )
...
def _is_table_duplicate_of_narrative(
    table: Table, page_elements: List[Element], threshold: float = 0.7
) -> bool:
    """
    ...
    page_narrative_text = " ".join(
        elem.text for elem in page_elements if elem.text and elem.element_type != ElementType.IMAGE
    )
...
def _stitch_tables(tables: List[Table], line_height: float = 20.0) -> List[Table]:
    ...
    sorted_tables = sorted(
        ...
        key=lambda t: (
            t.page_number,
            t.bbox.y0 if hasattr(t.bbox, "y0") else getattr(t.bbox, "y0", 0),
        ),
    )
    ...
    for next_table in sorted_tables[1:]:
        if _should_stitch_tables(current, next_table, line_height):
            ...
            current_y1 = (
                current.bbox.y1 if hasattr(current.bbox, "y1") else getattr(current.bbox, "y1", 0)
            )
            next_y1 = (
                next_table.bbox.y1
                if hasattr(next_table.bbox, "y1")
                else getattr(next_table.bbox, "y1", 0)
            )
            ...
            # Update metadata
            current.metadata["stitched_from"] = current.metadata.get("stitched_from", []) + [
                next_table.element_id
            ]
            logger.debug(
...
def _aligns_with_column(
    elem: Element, table: Table, col_index: int, threshold: float = 50.0
) -> bool:
    """
    ...
    table_width = (
        table.bbox.x1 if hasattr(table.bbox, "x1") else getattr(table.bbox, "x1", 0)
    ) - table_x0
...
def _promote_narrative_to_table(
    elements: List[Element], table: Table, line_height: float = 20.0
) -> Table:
    """
    ...
    for elem in elements:
        ...
        # Skip header rows
        if _is_header_row(
            [TableCell(row=0, col=0, text=elem.text, bbox=elem.bbox, confidence=0.9)]
        ):
            continue
        ...
        for row_data in promoted_rows:
            ...
            for col_idx, part_text in enumerate(row_parts):
                ...
                cell_bbox = create_bbox(
                    ...
                    table.page_number,
                    (
                        table.bbox.layout_width
                        if hasattr(table.bbox, "layout_width")
                        else getattr(table.bbox, "layout_width")
                    ),
                    (
                        table.bbox.layout_height
                        if hasattr(table.bbox, "layout_height")
                        else getattr(table.bbox, "layout_height")
                    ),
                )
...
def _extract_money_value(
    elements: List[Element], tables: List[Table], label: str
) -> Optional[float]:
    """
    ...
    for table in tables:
        if not table.metadata.get("is_decorative", False) and table.bbox:
            table_bottom = (
                table.bbox.y1 if hasattr(table.bbox, "y1") else getattr(table.bbox, "y1", 0)
            )
...
def _build_invoice_data(elements: List[Element], tables: List[Table]) -> Dict[str, Any]:
    ...
    COL_MAP = {0: "description", 1: "unit_price", 2: "quantity", 3: "total"}
    ...
    for elem in elements:
        ...
        for table in tables:
            ...
            for cell in table.cells:
                ...
                if cell_x0 <= elem_center_x <= cell_x1 and cell_y0 <= elem_center_y <= cell_y1:
