...
@pytest.mark.skipif(
    not Path("datasets/ORTHO case 1.pdf").exists(),
...
def test_layout_aware_scanned_pdf_needs_ocr():
    ...
    pdf_path = Path("datasets/ORTHO case 1.pdf")
...
def test_layout_aware_digital_pdf_no_ocr():
    ...
    assert (
        result_layout["needs_ocr"] is False
    ), "Layout-aware detection should identify no need for OCR"
